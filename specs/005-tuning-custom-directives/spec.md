...
- Q: After seeing tuning displayed in both locations on-device, does the double-display still make sense? → A: No — tuning should display only once, as the tag directly below the artist name. It is no longer duplicated in the metadata area.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - See a song's tuning at a glance (Priority: P1)

A user has tagged a ChordPro file with a tuning (e.g. "Drop D"). While viewing the song on stage, they see the tuning as a small tag directly under the artist name — visible the instant the song opens.

**Why this priority**: This is the core capability the feature exists to deliver. Musicians switching tunings between songs in a set need this to be immediately visible, not something they hunt for.

**Independent Test**: Import a ChordPro file containing a tuning directive, open the song, and confirm the tuning appears directly below the artist name.
...
2. **Given** the same file, **When** the user views the song, **Then** the tuning does NOT also appear in the song's metadata area — it displays in exactly one place (per Session 2026-07-17 Clarifications).
3. **Given** a ChordPro file with no tuning directive, **When** the user opens the song, **Then** no tuning tag appears.
4. **Given** a ChordPro file with a tuning directive but no key, capo, time signature, or preset, **When** the user opens the song, **Then** the metadata area does not appear at all — only the below-artist tuning tag is shown.
...
- **FR-003**: Tuning MUST NOT be duplicated within the song's metadata area — it displays only via the below-artist tag (FR-002), keeping the metadata area reserved for key/capo/time signature/preset (revised per Session 2026-07-17 Clarifications; originally required dual display).
...
- **FR-007**: The song's metadata area MUST appear whenever at least one of key, capo, time signature, or preset is present — not only when key/capo/time signature are present, as before this feature. Tuning does not affect this (FR-003).
...
- **FR-009**: A song with no tuning value MUST NOT display a tuning tag below the artist line.
...
- **SC-002**: 100% of songs containing a tuning directive display it exactly once, as the below-artist tag — never duplicated in the metadata area.
...
- The below-artist tuning tag and the metadata-area preset entry are each labeled plainly (e.g. "Tuning: Drop D", "Preset: Preset 3"), matching the existing "Key: …" / "Capo …" / "Time: …" chip style.
- The preset chip is appended after the existing key/capo/time-signature chips, preserving today's display order for existing songs. Tuning is not a metadata-area chip (Session 2026-07-17 Clarifications).
